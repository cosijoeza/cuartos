<?php
    include('paquete.php');
	include 'conex.php';
    $query = "select e.domicilio,e.costo,e.deposito,te.tipo AS edificio,ts.tipo AS servicio,a.nombre,a.contacto FROM edificio AS e INNER JOIN tipo_edificio AS te ON e.id_tipo=te.id INNER JOIN tipo_servicio AS ts ON e.id_servicio=ts.id INNER JOIN arrendatario AS a ON e.id_arrendatario=a.id";
    $resultados = false;
